...
The benchmark (see §4 and `docs/AI_ARCHITECTURE_AND_EVALUATION.md`) uses **public
onion-bulb images that are absent from both models' training and validation pools**:

- `evaluation/v1_eval/` — the **committed** fixed set: `images/healthy/` +
  `images/defective/` (15 images) and `manifest.csv` (`filename`, `subdir_note`,
  `scene`, `class`, `source_note`, `checksum_sha256`), generated by
  `scripts/make_eval_manifest.py` with computed SHA-256. Because the images are
  committed, the set is immutable and reproducible from any checkout.
- Labels are **two-class** (`healthy` / `defective`), assigned at collection time
  from search-query/source-page titles — **not pixel-annotated and with no
  inter-annotator agreement** (stated limitation, not hidden).
- For the 4-class model the benchmark maps `{disease, rotten, sprouted} → defective`
  and scores the top-1 prediction.
...
> The **authoritative** held-out benchmark is the committed 15-image set in
> `evaluation/v1_eval/`, scored by `scripts/evaluate_models.py` and documented
> in `docs/AI_ARCHITECTURE_AND_EVALUATION.md` §5. **On that set the legacy
> 4-class model is more accurate overall (0.733 vs 0.533) while the Arena binary
> model has strictly better defective recall (1.0 vs 0.40)** — see that document
> for the full confusion matrix, subgroup (single-bulb) results, latency, and
> reading of the finding.

For completeness, the earlier **7-image exploratory smoke set** measured:

| Metric | Legacy 4-class (`onion_classifier.pt`) | Arena binary (`best.pt`) |
| :--- | ---: | ---: |
| Accuracy (7 images, smoke) | 5 / 7 = **0.714** | 6 / 7 = **0.857** |
...
The 7-image result is **preliminary and must not be treated as a generalization
claim**; the 15-image committed set is the reference.
