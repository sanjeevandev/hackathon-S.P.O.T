"""
S.P.O.T. Phase 3 — Train YOLO11n-cls on Pilot Dataset
======================================================
Fine-tunes YOLO11n-cls (ImageNet-pretrained) on the 99-sample binary
pilot dataset using aggressive transfer learning on CPU.

Output: artifacts/ml/experiments/yolo11n_cls_pilot_v1/
"""
...
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "artifacts", "ml", "experiments", "yolo11n_cls_pilot_v1")
...
def main():
    print("=" * 70)
    print("S.P.O.T. Phase 3 — Train YOLO11n-cls")
    print("=" * 70)
    ...
    print(f"\n[2/4] Loading YOLO11n-cls pretrained model...")
    ...
    model = YOLO("yolo11n-cls.pt")  # Downloads if not cached
    print(f"  Model loaded: yolo11n-cls.pt")
    ...
    if not os.path.exists(best_model_path):
        # Fallback: look in the actual output directory
    ...
    class_names = ["defective", "healthy"]  # YOLO sorts alphabetically
    ...
    experiment_results = {
        "experiment_name": "YOLO11n-cls Pilot Binary Classification",
        "experiment_id": "yolo11n-cls-pilot-v1",
        ...
        "comparison_vs_baseline": {
            ...
            "note": "ResNet18 baseline was trained on 74-dim feature vectors (tabular MLP), not raw images. Architecturally different from YOLO11 CNN. Comparison is directional only.",
            ...
            "yolo11_accuracy": round(accuracy, 4),
            "yolo11_f1": round(macro_f1, 4),
            "yolo11_defect_recall": per_class_metrics["defective"]["recall"],
            "yolo11_model_size_mb": model_size_mb,
            "yolo11_latency_ms": round(avg_latency, 2),
        },
        "limitations": [
            "Trained on only 80 images (extremely small dataset)",
            "Single annotator labels — no IAA validation",
            "CPU-only training — GPU could improve convergence",
            "Binary classification loses multi-label granularity",
            "Metrics on 19-sample validation set have high variance",
            "Transfer learning heavily relied upon (ImageNet backbone frozen)",
        ],
    ...
    print(f"{'=' * 70}")
    print(f"{'Metric':<25} {'ResNet18 (baseline)':<22} {'YOLO11n-cls (new)':<22}")
    print(f"{'-'*25} {'-'*22} {'-'*22}")
